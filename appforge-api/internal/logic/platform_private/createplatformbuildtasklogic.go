...
func (l *CreatePlatformBuildTaskLogic) CreatePlatformBuildTask(req *types.CreatePlatformBuildTaskReq) (resp *types.PlatformBuildTaskResp, err error) {
	item, err := l.svcCtx.CoreCli.CreateBuildTask(l.ctx, &corepb.CreateBuildTaskReq{AppId: req.AppId, VersionId: req.VersionId, ChannelId: req.ChannelId, SigningConfigId: req.SigningConfigId, Priority: req.Priority})
	if err != nil {
		return nil, err
	}
