...
import {
  platformService,
  type PlatformBrandingPreflight,
  type PlatformBrandingProfile,
} from '@/services'
...
const preflightStatusText = (status: number) =>
  ({ 1: '处理中', 2: '兼容', 3: '不兼容', 4: '执行失败' })[status] || '未知'
...
async function changeStatus(row: PlatformBrandingProfile) {
  ...
  try {
    await ElMessageBox.confirm(
      `确定${status === 2 ? '启用' : '停用'}“${row.profileName}”吗？`,
      '状态确认',
    )
...
async function startPreflight(row: PlatformBrandingProfile) {
  try {
    ...
    if (error !== 'cancel' && error !== 'close')
      ElMessage.error(error instanceof Error ? error.message : '创建预检失败')
  }
}
...
async function loadPreflights() {
  ...
  try {
    const response = await platformService.listBrandingPreflights({
      brandingProfileId: preflightProfileId.value,
      limit: 100,
    })
    if (response.code !== 200) throw new Error(response.msg)
