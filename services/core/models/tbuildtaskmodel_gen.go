...
func (m *defaultTBuildTaskModel) Insert(ctx context.Context, data *TBuildTask) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.TenantId, data.AppId, data.VersionId, data.ChannelId, data.SigningConfigId, data.ChannelCode, data.VersionCode, data.VersionName, data.SourceApkObjectId, data.SourceApkUrl, data.BuildConfig, data.Status, data.BuilderId, data.BuilderAttempt, data.Priority, data.ApkObjectId, data.ApkUrl, data.ApkSha256, data.ApkSize, data.LogObjectId, data.LogUrl, data.ErrorMessage, data.QueuedAt, data.StartTime, data.FinishTime, data.LeaseUntil, data.CreateBy)
	}, tBuildTaskIdKey)
	return ret, err
}
...
func (m *defaultTBuildTaskModel) Update(ctx context.Context, data *TBuildTask) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.TenantId, data.AppId, data.VersionId, data.ChannelId, data.SigningConfigId, data.ChannelCode, data.VersionCode, data.VersionName, data.SourceApkObjectId, data.SourceApkUrl, data.BuildConfig, data.Status, data.BuilderId, data.BuilderAttempt, data.Priority, data.ApkObjectId, data.ApkUrl, data.ApkSha256, data.ApkSize, data.LogObjectId, data.LogUrl, data.ErrorMessage, data.QueuedAt, data.StartTime, data.FinishTime, data.LeaseUntil, data.CreateBy, data.Id)
	}, tBuildTaskIdKey)
	return err
}
