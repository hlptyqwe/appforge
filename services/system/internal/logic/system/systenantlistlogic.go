...
func (l *SysTenantListLogic) SysTenantList(in *system.SysTenantListReq) (*system.SysTenantListResp, error) {
	...
	items, total, err := l.svcCtx.TenantMode.FindPage(l.ctx, models.TenantPageFilter{
		Keyword: strings.TrimSpace(in.GetKeyword()), Status: int64(in.GetEnabled()), TenantCode: strings.TrimSpace(in.GetTenantCode()), TenantName: strings.TrimSpace(in.GetTenantName()), ContactName: strings.TrimSpace(in.GetContactName()), ContactPhone: strings.TrimSpace(in.GetContactPhone()), IDs: in.GetIds(),
	}, cursor, limit)
	if err != nil {
	...
	return &system.SysTenantListResp{Base: responsePage(total, next > 0, next), Data: data}, nil
}
