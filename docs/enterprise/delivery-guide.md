...
统一基础回归入口为 `deploy/acceptance/v1-v7-regression-base.sh`，要求 Node.js 20 或更高版本，并覆盖 11 个 Go 模块、统一前端类型检查、交付 Shell 语法、V4–V7 本地运行验收和限定规模并发性能冒烟。`deploy/acceptance/v7-artifact-capacity.sh` 另行覆盖合成真实 APK 并发构建、本地 MinIO 双向吞吐、逐对象 SHA-256 和短时 API 稳定性，并生成 0600 JSON 证据。`deploy/acceptance/v7-api-soak.sh` 与 `.github/workflows/hourly-api-soak.yml` 已在公开原生 `linux/amd64` 当前源码 Compose 上完成 3600 秒认证 API 长稳，health、ready 和带 JWT/RBAC/RPC/数据库读取的部署状态均满足独立 P99、最小请求数、零错误及零连续错误门禁，8 个核心容器零重启，证据见 `evidence/v7-api-soak-hour-public-ci-20260817.json`。上述门禁仍不替代 Android 授权真机、客户目标容量/峰值、生产规模 APK/对象存储吞吐、天级长稳或外部系统验收。
