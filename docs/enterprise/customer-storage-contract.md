...
客户现场先用 `deploy/local-agent/customer-storage-probe.sh ABSOLUTE_REPORT.json` 验证真实 Provider。探针必须复用已注册 Agent 状态和受限 `local-file://` Secret，只能在登记前缀的 `acceptance/storage-probe/<random>.bin` 写入一个合成对象，并完成 Put、Stat、完整 Get/SHA-256、Delete 和权威 NotFound 删除确认。它不得 List、读取既有 Key、创建 bucket 或修改 policy；证据不得包含 endpoint、bucket、prefix 原值和任何凭据，只记录这些目标的 SHA-256 指纹、耗时、合成对象摘要及删除状态。正式客户脚本把证据标记为 `environmentKind=customer-test`，仓库临时 Provider 夹具只能标记为 `fixture`，两者不得混用。客户测试策略因此需在登记前缀额外允许精确 DeleteObject。公开原生 `linux/amd64` 夹具 CI 和 `v1.2.7` 正式 Local Agent digest 版本门禁已通过，证据见 `evidence/v7-customer-storage-probe-release-v1.2.7-20260817.json`；现场探针通过仍不等于控制面登记与完整 APK 构建已经通过，后者必须继续执行本节完整 E2E。
