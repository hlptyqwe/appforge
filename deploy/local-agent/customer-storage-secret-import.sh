...
APPFORGE_CUSTOMER_STORAGE_SECRET_NAME="$secret_name" \
  docker compose --profile maintenance run --rm -T customer-storage-secret-import <"$secret_file"
echo "导入完成；Agent customer_storage_ref 使用命令输出的 local-file:// 引用"
