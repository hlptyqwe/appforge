...
project="appforge-v7-schema112-dr-$$"
...
[[ $project =~ ^appforge-v7-schema112-dr-[0-9]+$ ]]
...
awk '
  ...
  !excluded { print }
' "$repo_root/services/core/core.sql" >"$delivery/core-schema-112.sql"
! grep -q 't_air_gapped_package' "$delivery/core-schema-112.sql"

for migration in "$repo_root"/deploy/mysql/migrations/*.sql; do
  migration_name=$(basename "$migration")
  migration_number=${migration_name%%-*}
  if ((10#$migration_number <= 112)); then
...
(root / "synthetic-build.log").write_text(
    "synthetic AppForge Schema 112 disaster recovery build log\n",
    encoding="utf-8",
)
PY
...
if [[ ${APPFORGE_DR_FIXTURE_ONLY:-false} == true ]]; then
  ...
  grep -q 'CREATE TABLE t_app_application' "$delivery/core-schema-112.sql"
  grep -q 'CREATE TABLE t_build_task' "$delivery/core-schema-112.sql"
  [[ $source_size -gt 0 && $keystore_size -gt 0 && $apk_size -gt 0 && $log_size -gt 0 ]]
  echo "通过: Schema 112 截断、113 排除、迁移边界与合成 APK/Keystore/构建产物/日志 fixture 静态校验"
  exit 0
fi
...
business_summary() {
  mysql_query "
    ...
    SELECT CONCAT_WS('|','OBJECT',id,tenant_id,app_id,object_type,object_key,size_bytes,sha256,status,storage_mode,owner_agent_id) FROM t_storage_object WHERE id IN ($source_object_id,$keystore_object_id,$apk_object_id,$log_object_id) ORDER BY id;
...
mysql_file "$delivery/core-schema-112.sql"
...
VALUES
  ...
  ($log_object_id,$tenant_id,$app_id,4,'tenants/$tenant_id/apps/$app_id/logs/$task_id/build.log','build.log','text/plain',$log_size,'$log_sha256',3,1,0,970001);
...
VALUES
  ($version_id,$tenant_id,$app_id,112,'1.1.2-dr',$source_object_id,'storage-object://$source_object_id','$source_sha256','Schema 112 合成灾备版本',JSON_OBJECT('scenario','v7-dr'),2,970001);
...
VALUES
  ($task_id,$tenant_id,$app_id,$version_id,$channel_id,$signing_config_id,'DR_SYNTHETIC',112,'1.1.2-dr',$source_object_id,'storage-object://$source_object_id',JSON_OBJECT('scenario','v7-dr'),'isolated-dr','SUCCESS','synthetic-dr-builder',1,100,$apk_object_id,'storage-object://$apk_object_id','$apk_sha256',$apk_size,$log_object_id,'storage-object://$log_object_id','2026-08-15 00:00:00','2026-08-15 00:00:01','2026-08-15 00:00:02',970001);
...
VALUES
  ($audit_id,$tenant_id,970001,'v7-dr-owner','build','synthetic-success','POST','/core/build-tasks','{\"taskId\":$task_id}','{\"status\":\"SUCCESS\"}','127.0.0.1',112,1786723202000,1786723202000);
SQL
...
[[ $(printf '%s\n' "$before_business_summary" | grep -c '^OBJECT|') == 4 ]]
[[ $(printf '%s\n' "$before_business_summary" | grep -c '^REFS|1$') == 1 ]]
[[ $(printf '%s\n' "$before_object_summary" | grep -c .) == 4 ]]
...
[[ $before_business_summary == "$after_business_summary" ]] || {
  echo "验收失败: Schema 112 业务摘要恢复前后不一致" >&2
...
if [[ -n $report_file ]]; then
  ...
  umask 077
  APPFORGE_DR_SCENARIO=isolated-synthetic-appforge-schema112 \
...
json.dump({
  ...
  "acceptanceScript": "deploy/acceptance/v7-appforge-schema112-dr.sh",
  "targetDatabaseSchema": os.environ["APPFORGE_DR_SCHEMA_TARGET"],
  "candidateSchema113Excluded": True,
  ...
  "verifiedData": [
    ...
    "four-storage-object-metadata-and-bytes",
...
echo "通过: 隔离 AppForge Schema 112 合成业务链、etcd 与4个对象完成破坏恢复；Schema 113被拒绝；RPO=${rpo_seconds}s/${rpo_target_seconds}s，RTO=${rto_seconds}s/${rto_target_seconds}s"
